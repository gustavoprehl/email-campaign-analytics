...
select
    ...
    count(distinct if(tipo_evento = 'sent', id_subscriber, null)) as qtd_enviados,
    count(distinct if(tipo_evento = 'bounce', id_subscriber, null)) as qtd_bounces,
    count(distinct if(tipo_evento = 'sent', id_subscriber, null))
        - count(distinct if(tipo_evento = 'bounce', id_subscriber, null)) as qtd_entregues,
    count(distinct if(tipo_evento = 'open', id_subscriber, null)) as qtd_abertos,
    count(distinct if(tipo_evento = 'click', id_subscriber, null)) as qtd_clicks
